...
ggplot() + 
  ...
  layer(data=dfb, 
        mapping=aes(x=Y, y=AVG_CAMPAIGN), 
        stat="identity", 
        stat_params=list(), 
        geom="bar",
        geom_params=list(colour="blue", fill="blue"), 
        position=position_identity()
  ) + coord_flip() +
  layer(data=dfb, 
        mapping=aes(x=Y, y=AVG_CAMPAIGN, label=round(AVG_CAMPAIGN)), 
        stat="identity", 
        stat_params=list(), 
        geom="text",
  ...
  layer(data=dfb, 
        mapping=aes(x=Y, y=AVG_CAMPAIGN, label=round(WINDOW_AVG_CAMPAIGN)), 
        stat="identity", 
        stat_params=list(), 
        geom="text",
  ...
  layer(data=dfb, 
        mapping=aes(x=Y, y=AVG_CAMPAIGN, label=round(AVG_CAMPAIGN - WINDOW_AVG_CAMPAIGN)), 
        stat="identity", 
        stat_params=list(), 
        geom="text",
